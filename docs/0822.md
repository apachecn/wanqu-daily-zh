...
> 原文:[https://codewords . recurse . com/issues/two/git-from-the-inside-out？UTM _ source = Wanqu . co&UTM _ campaign = Wanqu+Daily&UTM _ medium = website](https://codewords.recurse.com/issues/two/git-from-the-inside-out?utm_source=wanqu.co&utm_campaign=Wanqu+Daily&utm_medium=website)
...
![Tree graph for the `a1` commit](../Images/d4ebd3a39543b6de76b2deb1b9475c13.png)
...
![`a1` commit object pointing at its tree graph](../Images/8e6e37a688270fbf54b65da1e5a908b1.png)
...
![`master` pointing at the `a1` commit](../Images/a64684d87614c1ee1ec32c286cda1262.png)
...
![`a1` commit shown with the working copy and index](../Images/fe1390e5cdcd33531cea05a379e2a724.png)
...
![`data/number.txt` set to `2` in the working copy](../Images/c6b798d6f83de0bc439671ae6f600ef0.png)
...
![`data/number.txt` set to `2` in the working copy and index](../Images/7e02fa6433a39c256011b6b510c9ebe6.png)
...
![`a2` commit](../Images/d794cd0e07470fe11c4ae3abe2a35f1d.png)

` a2 '提交

![Git graph without the working copy and index](../Images/ae22c2bfd82f591c5539b80bb83b32d1.png)
...
![Detached `HEAD` on `a2` commit](../Images/54b349ed8b33f4d5f50bb6a95f031b35.png)
...
![`a3` commit that is not on a branch](../Images/bdcab7f2208cd12373aa6a65dac17a39.png)
...
![`a3` commit now on the `deputy` branch](../Images/d2b18efdf771f28514ddf8b872c3647f.png)
...
![`master` checked out and pointing at the `a2` commit](../Images/29d78d40b0198bd962f0bc9c7f747276.png)
...
![`deputy` checked out](../Images/6fd76cea2249c9a2291c2acd2194ccac.png)
...
![`master` checked out and pointing at the `a2` commit](../Images/7083fd79a8422eac0c42c538e3a23696.png)
...
![`a3` commit from `deputy` fast-forward merged into `master`](../Images/bd11ca0570fc894b08cc7355477ff266.png)
...
![`a4` committed to `master`, `b3` committed to `deputy` and `deputy` checked out](../Images/a8ee370ff12ed46f13f37b52f12925f4.png)
...
![`a3`, the base commit of `a4` and `b3`](../Images/a86c58848cdf2b210adfcd1d9be89e7b.png)
...
![`b4`, the merge commit resulting from the recursive merge of `a4` into `b3`](../Images/a42eeaf9f4d83ecfad83e04ff26bf77a.png)
...
![`deputy` merged into `master` to bring `master` up to the latest commit, `b4`](../Images/629a3bd868ced14be5860c020d5a8284.png)
...
![`b5` commit on `deputy` and `b6` commit on `master`](../Images/5086be447824adb56fd5040c073388bc.png)
...
![`MERGE_HEAD` written during merge of `b5` into `b6`](../Images/8e152e8fe5a00e88c6aae01a2016d8d6.png)
...
![`b11`, the merge commit resulting from the conflicted, recursive merge of `b5` into `b6`](../Images/b0f5f34e96ff2e7fc64ac2da49bbae8f.png)
...
![The working copy, index, `b11` commit and its tree graph](../Images/15fd4b01b83ba38d4647b65f6f682037.png)
...
![After `data/letter.txt` `rm`ed from working copy and index](../Images/bde08c597cc4ec4065890765790a3150.png)
...
![`11` commit made after `data/letter.txt` `rm`ed](../Images/60f0767d096333fafe54d2da0679188c.png)
...
![New graph created when `alpha` `cp`ed to `bravo`](../Images/c79ca1749f094266cd747a4be78386d2.png)
...
![`12` commit on `bravo` repository](../Images/58dc7bed0a29ce93eb9507f75e994771.png)
...
![`alpha` after `bravo/master` fetched](../Images/8de0867d7ef4c69e437ca77d890c265c.png)
...
![`alpha` after `FETCH_HEAD` merged](../Images/37c3fb23f3d286c531c31b8bd867ec77.png)
...
![`alpha` and `delta` graphs after `alpha` cloned to `delta`](../Images/8cf569994d1641ea144c61f66ac124f2.png)
...
![`14` commit on `alpha`](../Images/6c005e46b1b3f0d0ee0478abbd1392da.png)
...
![`14` commit pushed from `alpha` to `delta`](../Images/d5964c94cc2ab4b766f031887ea24dc3.png)
