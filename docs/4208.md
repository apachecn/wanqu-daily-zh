...
> 原文:[http://www.paulgraham.com/avg.html?utm_source=wanqu.co&UTM _ campaign = Wanqu+Daily&UTM _ medium =网站](http://www.paulgraham.com/avg.html?utm_source=wanqu.co&utm_campaign=Wanqu+Daily&utm_medium=website)

![Beating the Averages](../Images/94df67845a11010dcaa4106177ad9ae4.png)

 <font size="2" face="verdana">*(本文摘自 2001 年 Franz 开发者研讨会上的一次演讲。)*

1995 年夏天，我和我的朋友 Robert Morris 创办了一家名为 [Viaweb](http://docs.yahoo.com/docs/pr/release184.html) 的初创公司。我们的计划是编写能让最终用户建立在线商店的软件。当时，这个软件的新颖之处在于它运行在我们的服务器上，使用普通的网页作为界面。当然，很多人可能同时有这个想法，但据我所知，Viaweb 是第一个基于网络的应用程序。对我们来说，这似乎是一个非常新颖的想法，所以我们用它来命名公司:Viaweb，因为我们的软件是通过网络工作的，而不是在你的台式电脑上运行。这个软件的另一个不寻常之处是它主要是用一种叫做 Lisp 的编程语言编写的。这是第一个用 Lisp 编写的大型终端用户应用程序之一，在此之前，Lisp 主要用于大学和研究实验室。[1]

**秘密武器**

埃里克·雷蒙德写了一篇名为《如何成为一名黑客》的文章，在这篇文章中，他告诉潜在的黑客们他们应该学习什么语言。他建议从 Python 和 Java 开始，因为它们简单易学。严肃的黑客还想学习 C 语言，以便破解 Unix，以及 Perl 语言，用于系统管理和 cgi 脚本。最后，真正严肃的黑客应该考虑学习 Lisp:

> Lisp 值得学习是因为你最终得到它时会有深刻的启蒙体验；这种经历会让你在以后的日子里成为一名更好的程序员，即使你实际上从来没有大量使用 Lisp 本身。
